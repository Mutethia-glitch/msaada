...
export default function CreateNeed() {
  ...
  const createCategory = trpc.categories.create.useMutation({
    onSuccess: (category) => { if (category) setCategoryId(category.id); setNewCategory(""); setShowNewCategory(false); void categories.refetch(); },
  });
  ...
  const addCategory = () => { const name = newCategory.trim(); if (name.length >= 2) createCategory.mutate({ name }); };
