...
export default function CreateNeed() {
  ...
  const errorsForStep = (currentStep: number, categoryOverride = categoryId): FieldError[] => {
    ...
    if (currentStep === 1) return [
      ...
      ...(!categoryOverride ? [{ label: "Category", message: "Choose a category or add a new one." }] : []),
    ];
  ...
  const next = () => { const domCategory = step === 1 ? Number((document.querySelector("select[name='category'], .structured-grid select") as HTMLSelectElement | null)?.value || 0) || undefined : categoryId; if (step === 1 && domCategory) setCategoryId(domCategory); const errors = errorsForStep(step, domCategory); setFieldErrors(errors); if (!errors.length) setStep(Math.min(3, step + 1)); };
