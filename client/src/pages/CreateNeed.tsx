...
export default function CreateNeed() {
  ...
  const createCategory = trpc.categories.create.useMutation({
    onSuccess: async (category) => {
      const refreshed = await categories.refetch();
      const created = category ?? refreshed.data?.find((item) => item.name.toLowerCase() === newCategory.trim().toLowerCase());
      if (created) {
        ...
        setCategoryError("Category was saved, but the list could not be refreshed. Please choose it again.");
      }
    },
    onError: (error) => setCategoryError(error.message || "Unable to add this category."),
  });
  ...
  const addCategory = () => { const name = newCategory.trim(); setCategoryError(""); if (name.length >= 2) createCategory.mutate({ name }); };
  ...
  return <div className="app-page create-page"><header className="app-header container"><Link href="/" className="brand"><span className="brand-mark">m</span><span className="draft-status">{saveDraft.isPending ? "Saving…" : "Your words are saved securely"}</span></Link><Link href="/discover" className="exit-link">Exit <ArrowLeft size={14} /></Link></header><main className="create-main container">{submitted ? <div className="success-state"><div className="success-mark"><Check size={28} /></div><span className="section-kicker">Submitted for review</span><h1>Your need is now<br /><em>in good hands.</em></h1><p>A Msaada moderator will review the details before the request becomes public. We will let you know when there is an update.</p><Link href="/dashboard" className="button button-dark">Go to my dashboard <ArrowRight size={17} /></Link></div> : <><div className="stepper">{steps.map((item, i) => <div className={`step ${i === step ? "active" : i < step ? "complete" : ""}`} key={item}><span>{i < step ? <Check size={13} /> : i + 1}</span><label>{item}</label></div>)}</div><div className="create-card">
    ...
    {step === 1 && <div className="create-step"><div className="step-heading"><span className="section-kicker">Step 2 / Shape the request</span><h1>Give your request<br /><em>a clear shape.</em></h1><p>Choose an existing category or create one that fits your community’s need.</p></div><div className="structured-grid"><label className="field"><span>Need title</span><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Desks for community school students" /></label><label className="field"><span>Category</span><select value={categoryId ?? ""} onChange={(e) => setCategoryId(e.target.value ? Number(e.target.value) : undefined)}><option value="">Choose a category</option>{categories.data?.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label><label className="field"><span>Urgency</span><select value={urgency} onChange={(e) => setUrgency(e.target.value as typeof urgency)}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></label><label className="field"><span>Beneficiaries</span><input type="number" min="0" value={beneficiaryCount} onChange={(e) => setBeneficiaryCount(Number(e.target.value))} /></label></div><button type="button" className="text-button category-add-toggle" onClick={() => { setCategoryError(""); setShowNewCategory(!showNewCategory); }}>{showNewCategory ? <><X size={15} /> Cancel new category</> : <><Plus size={15} /> Add a new category</>}</button>{showNewCategory && <div className="new-category-row"><input value={newCategory} onChange={(e) => setNewCategory(e.target.value)} placeholder="e.g. Disability access" maxLength={80} /><button type="button" className="button button-outline" onClick={addCategory} disabled={newCategory.trim().length < 2 || createCategory.isPending}>{createCategory.isPending ? "Adding…" : "Add category"}</button></div>}{categoryError && <p className="category-error" role="alert">{categoryError}</p>}</div>}
    ...
    {fieldErrors.length > 0 && <div className="form-errors" role="alert"><strong>Please complete the following:</strong><ul>{fieldErrors.map((error) => <li key={error.label}><b>{error.label}:</b> {error.message}</li>)}</ul></div>}
    <div className="create-footer"><button className="text-button" onClick={() => { setFieldErrors([]); if (step > 0) setStep(step - 1); }}>{step > 0 && <><ArrowLeft size={15} /> Back</>}</button><div><button className="button button-ghost" onClick={() => persist(false)} disabled={!canSave || saveDraft.isPending}>Save draft</button>{step < 3 ? <button className="button button-dark" onClick={next}>Continue <ArrowRight size={16} /></button> : <button className="button button-dark" onClick={() => persist(true)} disabled={!canSave || saveDraft.isPending}>Submit for review <ArrowRight size={16} /></button>}</div></div>
  </div></>}</main></div>;
}
